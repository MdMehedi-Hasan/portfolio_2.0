...
export default function Skills() {
    const skills = [
        ...
        { title: 'Dev tools', level: '', icon: <BsBrowserChrome className='text-6xl' /> },
        { title: 'Github', level: '', icon: <IoLogoGithub className='text-6xl' /> }
    ]
    return (
        <div>
            <span className='flex items-center gap-2 text-xs border border-[#565656] rounded-full w-fit px-4 py-2'><LiaShapesSolid />SKILLS</span>
            <h1>My Skills</h1>
            <div className='grid grid-cols-5 gap-y-10'>
                {
                    skills.map((item, index) => <div key={index} className='flex flex-col items-center gap-2'>
                        <span className='border border-red-400 rounded-full py-16 px-10'>{item.icon}</span>
                        {item.title}
                    </div>)
                }
            </div>
